...
int aes_setkey_enc(int rk[], const unsigned char cipherKey[], int keyBytes);
int AesGenKeySched(int rk[], int rrk[], const unsigned char cipherKey[], int keyBytes);
...
int AesCtxIni(AesCtx *pCtx, unsigned char *pIV, unsigned char *pKey, int KeyLen, unsigned char Mode);
int AesEncrypt(AesCtx *pCtx, unsigned char *pData, unsigned char *pCipher, int DataLen);
int AesDecrypt(AesCtx *pCtx, unsigned char *pCipher, unsigned char *pData, int CipherLen);
void test(void);
...
void test()
{
    ...
    while(1) 
    {
	    ...
	    AesEncrypt(&ctx, databuf, ciph, sizeof(databuf));
    }

    
    // initialize context and decrypt cipher at other end
//print_str("Encrypted Data length : %d\n", strlen(databuf)+1);
/*
#ifdef DECRYPTION
    if( AesCtxIni(&ctx, iv, key, KEY128, CBC) < 0)
        print_str("init error\n");

    if (AesDecrypt(&ctx, databuf, databuf, sizeof databuf) < 0)
        print_str("error in decryption\n");

    print_str("Data : %s\n", databuf);
#endif
  */  
    /*	end = or1k_timer_get_ticks();
  __asm__ __volatile__("l.movhi r3,hi(0x91000000)\n"
        "l.movhi r4, 0\n"
        "l.addi r4,r0,0x0\n"
        "l.addi r5,r0,0xff\n"
        "l.sb 0x0(r3),r4\n"
