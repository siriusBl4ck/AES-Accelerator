...
int aes_setkey_enc(int rk[], const  char cipherKey[], int keyBytes);
int AesGenKeySched(int rk[], int rrk[], const  char cipherKey[], int keyBytes);
void AesEncBlk(AesCtx *pCtx, const  char pt[],  char ct[]);
void AesDecBlk(AesCtx *pCtx, const  char ct[],  char pt[]);
int AesCtxIni(AesCtx *pCtx,  char *pIV,  char *pKey, int KeyLen,  char Mode);
int AesEncrypt(AesCtx *pCtx,  char *pData,  char *pCipher, int DataLen);
int AesDecrypt(AesCtx *pCtx,  char *pCipher,  char *pData, int CipherLen);
void test(void);
...
#define GETU32(pt) (((int)(pt)[0] << 24) ^ \
                    ((int)(pt)[1] << 16) ^ \
                    ((int)(pt)[2] <<  8) ^ \
                    ((int)(pt)[3]))

#define PUTU32(ct, st) { (ct)[0] = ( char)((st) >> 24); \
                         (ct)[1] = ( char)((st) >> 16); \
                         (ct)[2] = ( char)((st) >>  8); \
                         (ct)[3] = ( char)(st); }
...
void AesEncBlk(AesCtx *pCtx, const  char pt[],  char ct[])
{
    int s0, s1, s2, s3, t0, t1, t2, t3, *iv;
    const int *rk;
    int r;
...
void AesDecBlk(AesCtx *pCtx, const  char ct[],  char pt[])
{
    int s0, s1, s2, s3, t0, t1, t2, t3, v0, v1, v2, v3, *iv;
    const int *rk;
    int r;
...
void hello(void)
{
    ...
    int databuf[4];
    databuf[0] = 0x3243f6a8;
    databuf[1] = 0x885a308d;
    databuf[2] = 0x313198a2;
    databuf[3] = 0xe0370734;
	...
	print_str("\n\nAnd now in hardware: \n");
	print_str("\n");
	AesCtxIni(&ctx, (char*)iv, (char*)key, KEY128, CBC);
	AesEncrypt(&ctx, (char*)databuf, (char*)ciph, sizeof(databuf));
	print_hex((int)(*ciph), sizeof(databuf));
